...
// Novelty motors appended after the real catalogue. "Bye Bye Legs" is absurdly
// overpowered (impulse ~10x any rocket's rating -> ~90% CATO on the pad); if it
// does light, `chaos` throws it off in a semi-random direction and the huge
// thrust sends it absurdly high. avgThrust === impulse / burnTime is kept so the
// catalog consistency test stays green.
const EXTRA_MOTORS = [
  { id: 'Bye Bye Legs', class: 'G', totalImpulseNs: 1200, avgThrustN: 300, burnTimeS: 4, massTotalKg: 0.30, massPropKg: 0.20, delayS: 2, chaos: 45 },
];
...
function motorsFile(motors) {
  const rows = motors.map((m) =>
    `  { id: ${JSON.stringify(m.id)}, class: ${JSON.stringify(m.class)}, totalImpulseNs: ${m.totalImpulseNs}, avgThrustN: ${m.avgThrustN}, burnTimeS: ${m.burnTimeS}, massTotalKg: ${m.massTotalKg}, massPropKg: ${m.massPropKg}, delayS: ${m.delayS}${m.chaos !== undefined ? `, chaos: ${m.chaos}` : ''} },`);
